...
```mermaid
flowchart TB
    REDUCE["Classical Reduction"] --> SEARCH["Quantum Search"]
    SEARCH --> COMBINE["Combined Assignment"]
    REDUCE -.- NOTE["Polynomial Time"]
    SEARCH -.- NOTE2["O(sqrt of 2^n) Queries"]
    COMBINE -.- NOTE3["Pr of success >= 1/2"]
```
...
```mermaid
flowchart TB
    B["Basic Layer"] --> Q["Quantum Layer"]
    Q --> P["Proofs Layer"]
    G["Gnostic P != NP"] --> CM["Covenant Mapping"]
    CM --> T["Theoretical Synthesis"]
    AA["Albert Algebra"] --> EB["Entropy Bomb"]
    EB --> DI["DMS Irreversibility"]
    BK["Boolean Kernel"] --> SV["Sentry Vectors"]
    P -.-> G
    CM -.-> P
    DI -.-> G
```
...
|   |
|   |-- Axioms.lean           reduce_sound, quantum_correct, reduce_extends_all
|
|-- Quantum/
...
|   |                          conservation_of_mystery, gnostic_p_ne_np
|   |
...
|                              F4 automorphism group, NunSpace
|
|-- DMS/
|   |-- EntropyBomb.lean      SBK split, Heartbeat
|   |                          symmetry_collapse, entropy_bomb_irreversible
|   |
|   |-- DMSIrreversibility.lean  dms_irreversibility_iff_P_ne_NP
|
|-- Security/
...
|                              GhostKey, master_seed, poison_pill_activates
|
...
| Theorem | File | Statement |
|---------|------|-----------|
| `nand_not_correct` | `Security/BooleanKernel.lean` | NAND(x,x) = NOT(x) |
| `nand_and_correct` | `Security/BooleanKernel.lean` | NAND(NAND(a,b),NAND(a,b)) = AND(a,b) |
| `nand_or_correct` | `Security/BooleanKernel.lean` | NAND(NAND(a,a),NAND(b,b)) = OR(a,b) |
| `nand_implies_correct` | `Security/BooleanKernel.lean` | OR(NOT(a),b) = IMPLIES(a,b) |
| `nand_equal_correct` | `Security/BooleanKernel.lean` | AND(IMPLIES(a,b),IMPLIES(b,a)) = EQUAL(a,b) |
